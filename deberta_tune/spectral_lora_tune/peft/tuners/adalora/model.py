        ...
        if (getattr(outputs, "loss", None) is not None) and isinstance(outputs.loss, torch.Tensor):
            ...
            for n, p in self.model.named_parameters():
                if ("lora_A" in n or "lora_B" in n) and self.trainable_adapter_name in n:
                    ...
                    num_param += 1
                    # print(f'{para_cov.device=}')
                    # print(f'{I.device=}')
                    # I = I.to(para_cov.device)
                    # print(f'{(para_cov@torch.linalg.pinv(para_cov)).diag().diag().clone().detach()=}')
                    # raise Exception('**')
                    try:
                        regu_loss += torch.norm(para_cov - I, p="fro")
                    except:
                        # print(regu_loss,para_cov.device,I.device)
                        # print(torch.norm(para_cov - I, p="fro"))
                        # print(torch.norm(para_cov - I, p="fro").to(regu_loss.device))
                        # print(regu_loss.device)
                        regu_loss += torch.norm(para_cov - I, p="fro").to(regu_loss.device)
                        # raise Exception('*')
            if num_param > 0:
