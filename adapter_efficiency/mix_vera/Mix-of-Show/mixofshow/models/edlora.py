...
class VeraLinearLayer(nn.Module):
    def __init__(self, name, original_module, rank=4, alpha=1):
        ...
        self.lora_down = nn.Parameter(torch.zeros(rank,in_features), requires_grad=False)
        self.lora_up = nn.Parameter(torch.zeros(out_features,rank), requires_grad=False)
        self.lora_d = nn.Parameter(torch.ones(rank), requires_grad=True)
        self.lora_b = nn.Parameter(torch.zeros(out_features),requires_grad=True)

        torch.nn.init.kaiming_uniform_(self.lora_down, a=math.sqrt(5))
        torch.nn.init.kaiming_uniform_(self.lora_up, a=math.sqrt(5))
    ...
    def forward(self, hidden_states):
        vera_weight = self.lora_b.diag()@self.lora_up@self.lora_d.diag()@self.lora_down
        hidden_states = self.original_forward(hidden_states) + self.alpha * F.linear(hidden_states, vera_weight, bias=None)
        return hidden_states