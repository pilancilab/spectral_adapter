...
def merge_vera_into_weight(original_state_dict, lora_state_dict, model_type, alpha):
    ...
    for k in new_state_dict.keys():
        ...
        d_name = lora_down_name.replace('lora_down', 'lora_d')
        b_name = lora_down_name.replace('lora_down', 'lora_b')
        if lora_down_name in lora_state_dict:
            ...
            d_params = lora_state_dict[d_name].to(original_params.device)
            b_params = lora_state_dict[b_name].to(original_params.device)
            if len(original_params.shape) == 4:
                raise Exception('')
            else:
                vera_weight = b_params.diag()@lora_up_params@d_params.diag()@lora_down_params
                new_state_dict[k] = original_params+alpha*vera_weight
...
def convert_edlora(pipe, state_dict, enable_edlora, alpha=0.6):
    ...
    updated_unet_state_dict = merge_vera_into_weight(pretrained_unet_state_dict, unet_lora_state_dict, model_type='unet',  alpha=alpha)
    ...
    updated_text_encoder_state_dict = merge_vera_into_weight(pretrained_text_encoder_state_dict, text_encoder_lora_state_dict, model_type='text_encoder', alpha=alpha)
