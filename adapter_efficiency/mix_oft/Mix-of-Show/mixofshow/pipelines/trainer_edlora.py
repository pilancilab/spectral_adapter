...
from mixofshow.models.edlora import (OFTLinearLayer, revise_edlora_unet_attention_controller_forward, revise_edlora_unet_attention_forward)
...
class EDLoRATrainer(nn.Module):
    ...
    def set_finetune_cfg(self, finetune_cfg):
                ...
                if module.__class__.__name__ == where:
                    for child_name, child_module in module.named_modules():
                        if child_module.__class__.__name__ == 'Linear':
                            lora_module = OFTLinearLayer(name + '.' + child_name, child_module, **text_encoder_cfg['lora_cfg'], block_share=True, oft_r=text_encoder_cfg['lora_cfg']['rank'])
                            for n,p in lora_module.named_parameters():
                                if p.requires_grad:
                                    if len(p.shape)==2:
                                        num_train+= p.shape[0]*p.shape[1]
                                    elif len(p.shape)==1:
                                        num_train+=p.shape[0]
                                    else:
                                        num_train+=p.shape[0]*p.shape[1]*p.shape[2]
                ...
                if module.__class__.__name__ == where:
                    for child_name, child_module in module.named_modules():
                        if child_module.__class__.__name__ == 'Linear' or (child_module.__class__.__name__ == 'Conv2d' and child_module.kernel_size == (1, 1)):
                            lora_module = OFTLinearLayer(name + '.' + child_name, child_module, **unet_cfg['lora_cfg'], block_share=True, oft_r=unet_cfg['lora_cfg']['rank'])
        ...
        print(f'# trainable params: {num_train}')
