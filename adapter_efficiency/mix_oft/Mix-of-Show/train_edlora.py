...
def train(root_path, args):
    ...
    optimize_params = EDLoRA_trainer.get_params_to_optimize()
    optimizer = torch.optim.AdamW(optimize_params, **train_opt['optim_g'])

    # print(EDLoRA_trainer.get_params_to_optimize())
    # print(optimize_params[0])
    # raise Exception('ppp')

    # for n,p in EDLoRA_trainer.unet.named_parameters():
    #     print(n, p.shape)

    # for n,p in EDLoRA_trainer.text_encoder.named_parameters():
    #     print(n, p.shape)

    
    # print(len(optimize_params))
    # print(optimize_params[0]['params'][0].shape)
    ...
    while global_step < opt['train']['total_iter']:
        ...
        with accelerator.accumulate(EDLoRA_trainer):
            ...
            loss = EDLoRA_trainer(batch['images'], batch['prompts'], masks, None)
            loss_dict['loss'] = loss
    ...
    accelerator.wait_for_everyone()
    save_and_validation(accelerator, opt, EDLoRA_trainer, val_dataloader, 'latest', logger, oft_r=opt['models']['finetune_cfg']['unet']['lora_cfg']['rank'])


def save_and_validation(accelerator, opt, EDLoRA_trainer, val_dataloader, global_step, logger, oft_r):
    ...
    if opt['val']['val_during_save']:
        ...
        for lora_alpha in opt['val']['alpha_list']:
            ...
            pipe, new_concept_cfg = convert_edlora(pipe, torch.load(save_path), enable_edlora=enable_edlora, alpha=lora_alpha, oft_r=oft_r)
