...
def merge_lilora_into_weight(original_state_dict, lora_state_dict, model_type, alpha):
    ...
    for k in new_state_dict.keys():
        ...
        down_aux_name = lora_down_name.replace('lora_down', 'down_aux')
        up_aux_name = lora_down_name.replace('lora_down', 'up_aux')
        if lora_down_name in lora_state_dict:
            ...
            down_aux_params = lora_state_dict[down_aux_name].to(original_params.device)
            up_aux_params = lora_state_dict[up_aux_name].to(original_params.device)
            if len(original_params.shape) == 4:
                raise Exception('')
            else:
                new_state_dict[k] = original_params+alpha*(up_aux_params@lora_up_params@lora_down_params@down_aux_params)
...
def convert_edlora(pipe, state_dict, enable_edlora, alpha=0.6):
    ...
    updated_unet_state_dict = merge_lilora_into_weight(pretrained_unet_state_dict, unet_lora_state_dict, model_type='unet',  alpha=alpha)
    ...
    updated_text_encoder_state_dict = merge_lilora_into_weight(pretrained_text_encoder_state_dict, text_encoder_lora_state_dict, model_type='text_encoder', alpha=alpha)
