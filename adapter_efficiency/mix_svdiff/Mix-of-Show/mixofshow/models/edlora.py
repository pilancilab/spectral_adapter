...
class SVDiffLinearLayer(nn.Module):
    def __init__(self, name, original_module, rank=4, alpha=1, top=True, idx=0):
        ...
        self.spectral_C = torch.nn.Parameter(torch.zeros(self.S.shape[0]))
        original_module.forward = self.forward
        self.original_module = original_module

    def forward(self, hidden_states):
        pad_S = torch.nn.functional.relu(self.S + self.spectral_C)
        pad_W = self.U@pad_S.diag()@self.V.T
        if self.conv :
            raise Exception('')
        else:
