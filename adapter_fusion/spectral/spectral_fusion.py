...
def merge_spectral_into_weight(original_state_dict, lora_state_dict, modification_layer_names, model_type, alpha, device, idx=0, weight_dict=None, concept_name=''):
    ...
    for k in modification_layer_names:
        ...
        if spectral_B_name in lora_state_dict:
            if (weight_dict is None) or (len(weight_dict.keys())==0):
                load_cnt += 1
                original_params = new_state_dict[k]
                spectral_A_params = lora_state_dict[spectral_A_name].to(device)
                spectral_B_params = lora_state_dict[spectral_B_name].to(device)
                spectral_C_params = lora_state_dict[spectral_C_name].to(device)
                U_params = lora_state_dict[U_name].to(device)
                S_params = lora_state_dict[S_name].to(device)
                V_params = lora_state_dict[V_name].to(device)
                lora_rank = min(spectral_A_params.shape)
                U_params += alpha*torch.nn.ConstantPad1d((idx*lora_rank,U_params.shape[1]-spectral_A_params.shape[1]-idx*lora_rank),0)(spectral_A_params)
                S_params += alpha*torch.nn.ConstantPad1d((idx*lora_rank,S_params.shape[0]-spectral_C_params.shape[0]-idx*lora_rank),0)(spectral_C_params)
                V_params += alpha*torch.nn.ConstantPad1d((idx*lora_rank,V_params.shape[1]-spectral_B_params.shape[1]-idx*lora_rank),0)(spectral_B_params)
                # print(f'{alpha=}')
                # AA = (1.44*spectral_A_params)@(1.44*spectral_C_params).diag()@(1.44*spectral_B_params).T
                # BB = (1*spectral_A_params)@((1.44**3)*spectral_C_params).diag()@(1*spectral_B_params).T
                # print('aaaaa ', torch.linalg.norm(AA-BB))
                # raise Exception('****')
                if len(original_params.shape) == 4:
                    raise Exception('here at line 149 in spectral_fuison.py')
                else:
                    pass
                spectral_param = U_params@S_params.diag()@V_params.T
                new_state_dict[k] = spectral_param
            else:
                print('matrix norm weight in merge_spectral')
                load_cnt += 1
                original_params = new_state_dict[k]
                spectral_A_params = lora_state_dict[spectral_A_name].to(device)
                spectral_B_params = lora_state_dict[spectral_B_name].to(device)
                ...
                V_params = lora_state_dict[V_name].to(device)
                lora_rank = min(spectral_A_params.shape)
                U_params += weight_dict[(concept_name,k)]*torch.nn.ConstantPad1d((idx*lora_rank,U_params.shape[1]-spectral_A_params.shape[1]-idx*lora_rank),0)(spectral_A_params)
                S_params += weight_dict[(concept_name,k)]*torch.nn.ConstantPad1d((idx*lora_rank,S_params.shape[0]-spectral_C_params.shape[0]-idx*lora_rank),0)(spectral_C_params)
                V_params += weight_dict[(concept_name,k)]*torch.nn.ConstantPad1d((idx*lora_rank,V_params.shape[1]-spectral_B_params.shape[1]-idx*lora_rank),0)(spectral_B_params)
                if len(original_params.shape) == 4:
                    raise Exception('')
                else:
                    pass
...
def parse_new_concepts(concept_cfg):
    ...
    for model_path in model_paths:
        ...
        if 'unet' in model and len(model['unet']) != 0:
            crosskv_matches = ['attn2.to_k.spectral', 'attn2.to_v.spectral','attn2.to_k.U','attn2.to_k.S','attn2.to_k.V',
                               'attn2.to_v.U', 'attn2.to_v.S', 'attn2.to_v.V']
            crosskv_dict = {
                k: v
...
def merge_kv_in_cross_attention(concept_list, optimize_iters, new_concept_cfg,
    ...
    for concept, tuned_state_dict in zip(concept_list, unet_crosskv_list):
        for layer_idx, layer_name in cross_kv_layer_names:
            spectral_A_name = layer_name.replace('to_k.weight', 'to_k.spectral_A').replace('to_v.weight', 'to_v.spectral_A')
            spectral_B_name = spectral_A_name.replace('spectral_A', 'spectral_B')
            spectral_C_name = spectral_A_name.replace('spectral_A', 'spectral_C')
            U_name = spectral_A_name.replace('spectral_A', 'U')
            S_name = spectral_A_name.replace('spectral_A', 'S')
            V_name = spectral_A_name.replace('spectral_A', 'V')
            spectral_A_params = tuned_state_dict[spectral_A_name].to(device)
            spectral_B_params = tuned_state_dict[spectral_B_name].to(device)
            spectral_C_params = tuned_state_dict[spectral_C_name].to(device)
            U_params = tuned_state_dict[U_name].to(device)
            S_params = tuned_state_dict[S_name].to(device)
            V_params = tuned_state_dict[V_name].to(device)
            lora_rank = min(spectral_A_params.shape)
            
            weight[(concept['concept_name'],layer_name)] = torch.linalg.norm(spectral_A_params@spectral_C_params.diag()@spectral_B_params.T, ord=torch.inf)
    
    for _, layer_name in cross_kv_layer_names:
        pass
    ...
    for concept, tuned_state_dict in zip(concept_list, unet_crosskv_list):
        ...
        for layer_idx, layer_name in cross_kv_layer_names:
            ...
            if len(weight.keys())==0:
                ...
                print('matrix norm weights in merge_kv')
                U_params += weight[(concept['concept_name'],layer_name)]*torch.nn.ConstantPad1d((idx*lora_rank,U_params.shape[1]-spectral_A_params.shape[1]-idx*lora_rank),0)(spectral_A_params)
                S_params += weight[(concept['concept_name'],layer_name)]*torch.nn.ConstantPad1d((idx*lora_rank,S_params.shape[0]-spectral_C_params.shape[0]-idx*lora_rank),0)(spectral_C_params)
                V_params += weight[(concept['concept_name'],layer_name)]*torch.nn.ConstantPad1d((idx*lora_rank,V_params.shape[1]-spectral_B_params.shape[1]-idx*lora_rank),0)(spectral_B_params)

            merge_params = U_params@S_params.diag()@V_params.T
...
def merge_text_encoder(concept_list, optimize_iters, new_concept_cfg,
    ...
    LoRA_keys = set([
        key.replace('.spectral_A', '.weight').replace('.spectral_B', '.weight').replace('.spectral_C', '.weight').replace('.S', '.weight').replace('.U', '.weight').replace('.V', '.weight').replace('.original_module.weight', '.weight').replace('.original_module.bias', '.weight')
    ...
    for concept, lora_state_dict in zip(concept_list, text_encoder_list):
        for layer_name in text_encoder_layer_names:
            spectral_A_name = layer_name.replace('q_proj.weight', 'q_proj.spectral_A') \
                .replace('k_proj.weight', 'k_proj.spectral_A') \
                .replace('v_proj.weight', 'v_proj.spectral_A') \
                .replace('out_proj.weight', 'out_proj.spectral_A') \
                .replace('fc1.weight', 'fc1.spectral_A') \
    ...
    weight = {}
    for concept, lora_state_dict in zip(concept_list, text_encoder_list):
        merged_state_dict = merge_spectral_into_weight(
            original_state_dict,
            lora_state_dict,
            text_encoder_layer_names,
            model_type='text_encoder',
            alpha=concept['text_encoder_alpha'],
            device=device, idx = concept['text_encoder_idx'], weight_dict=weight, concept_name=concept['concept_name'])
...
def merge_spatial_attention(concept_list, optimize_iters, new_concept_cfg, tokenizer, text_encoder, unet, unet_spatial_attn_list, test_scheduler, device):
    ...
    weight = {}
    for concept, lora_state_dict in zip(concept_list, unet_spatial_attn_list):
        for layer_name in spatial_attention_layer_names:
            spectral_A_name = layer_name.replace('to_q.weight', 'to_q.spectral_A') \
                .replace('to_k.weight', 'to_k.spectral_A') \
                .replace('to_v.weight', 'to_v.spectral_A') \
                .replace('to_out.0.weight', 'to_out.0.spectral_A') \
                .replace('ff.net.0.proj.weight', 'ff.net.0.proj.spectral_A') \
    ...
    for concept, tuned_state_dict in zip(concept_list, unet_spatial_attn_list):
        ...
        merged_state_dict = merge_spectral_into_weight(
            ...
            model_type='unet',
            alpha=concept['unet_alpha'],
            device=device, idx=concept['unet_idx'], weight_dict=weight, concept_name=concept['concept_name'])
