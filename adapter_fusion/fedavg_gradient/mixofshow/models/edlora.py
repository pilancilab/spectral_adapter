...
class LoRALinearLayer(nn.Module):
    ...
    def forward(self, hidden_states):
        hidden_states = self.original_forward(hidden_states) + self.alpha * self.lora_up(self.lora_down(hidden_states)) #F.linear(hidden_states,weight=self.lora_up@self.lora_down,bias=None)#self.alpha * self.lora_up(self.lora_down(hidden_states))
        return hidden_states
