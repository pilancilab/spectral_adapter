...
if [ ${sample_dog} -eq 1 ]
then
  fused_model="experiments/fedavg_fusion/chilloutmix/lora_animal/combined_model_base"
  ...
  CUDA_VISIBLE_DEVICES="0" python regionally_controlable_sampling.py \
    ...
    --save_dir="results/multi-concept/fedavg_animal_playground" \
...
then
  ...
  CUDA_VISIBLE_DEVICES="0" python regionally_controlable_sampling.py \
    ...
    --keypose_adaptor_weight=${keypose_adaptor_weight}\
    --keypose_condition=${keypose_condition} \
    --save_dir="results/multi-concept/fedavg_animal_galaxy" \
    --prompt="${context_prompt}" \
    --negative_prompt="${context_neg_prompt}" \
    --prompt_rewrite="${prompt_rewrite}" \
    --suffix="baseline" \
...
then
  ...
  CUDA_VISIBLE_DEVICES="0" python regionally_controlable_sampling.py \
    ...
    --keypose_adaptor_weight=${keypose_adaptor_weight}\
    --keypose_condition=${keypose_condition} \
    --save_dir="results/multi-concept/fedavg_animal_fuji" \
    --prompt="${context_prompt}" \
    --negative_prompt="${context_neg_prompt}" \
    --prompt_rewrite="${prompt_rewrite}" \
    --suffix="baseline" \
